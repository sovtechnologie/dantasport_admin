...
const EditVendorInfoForm = () => {
    ...
    useEffect(() => {
        if (vendorData?.result?.[0]) {
            ...
            form.setFieldsValue({
                ...
                aadharNumber: vendor.aadhaar_number || "6345-3475-6349",
    ...
    const handleFinish = (values) => {
        console.log("form valyes", values, values.bankDoc?.url);
        const otpValue = values.otp;
        if (!otpId) {
            // Set error on the OTP field
            form.setFields([
                {
                    name: "otp",
            ...
            return; // stop the submit handler
        }
        if (!otpValue) {
        ...
        if (!otpId) {
            form.setFields([
                { name: "otp", errors: ["Please enter correct otp"] }
            ]);
        }
        ...
        if (values.aadharDoc?.[0]?.originFileObj) {
            updateFormData.append("aadharDoc", values.aadharDoc?.[0]?.originFileObj);
        }


        console.log("formData", values.gstDocument?.[0]?.originFileObj, values.pancardDoc?.[0]?.originFileObj, values.bankDoc?.[0]?.originFileObj);
        ...
        <div className="vendor-form-wrapper">
            ...
            >
                ...
                <Row gutter={24}>
                    ...
                    <Col span={12}>
                        <Form.Item label="Enter Aadhar Number" name="aadharNumber" rules={[
