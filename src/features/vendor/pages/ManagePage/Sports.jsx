...
const SportsPage = () => {
  ...
  const {
    ...
    error,
  } = useFetchVenueSport({ vendorId: id, venueId: selectedVenueId });
  ...
  const tableData = sportsData?.result?.map((sport) => ({
    ...
    timing: `${moment(sport.from_time, "HH:mm:ss").format(
      "hh:mm A"
    )} - ${moment(sport.end_time, "HH:mm:ss").format("hh:mm A")}`,
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          ...
          onClick={() =>
            navigate(`/vendor/manage/editsport/${record.venueId}/${record.key}`)
          }
        />
      ),
    },
