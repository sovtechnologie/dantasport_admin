...
const GymDiscountPage = () => {
  ...
  // Fetch coupons data using getCouponList API for gym (type: 3)
  const { data: couponsData, loading: couponsLoading, error: couponsError, isFetching: couponsFetching, refetch: refetchCoupons } = useGetCouponList(id, 3);
  ...
  const dataSource = useMemo(() => {
    ...
    return couponsData.result.map(coupon => ({
      key: coupon.id,
      id: coupon.id,
      gym: coupon.gym?.gym_name || 'All Gyms', // Since gym_id can be null for all gyms
      gymId: coupon.linked_id, // Using linked_id as gymId
  ...
  useEffect(() => {
    if (couponsError) {
      message.error("Failed to load gym coupons");
    }
  }, [couponsError]);

  // Refetch coupons when component mounts (user navigates back from add/edit forms)
  useEffect(() => {
    console.log("🔄 Component mounted, refetching gym coupons...");
    refetchCoupons();
  }, [refetchCoupons]);
  ...
  const handleAddCoupon = useCallback(() => {
    navigate('/vendor/gym/adddiscount');
  }, [navigate]);
  ...
  const handleDeleteCoupon = useCallback(async (coupon) => {
    try {
      ...
      if (response.status === 200) {
        message.success("Gym coupon deleted successfully");
        // Explicitly refetch the coupons list
        console.log("🔄 Refreshing gym coupons list after delete...");
        await refetchCoupons();
        message.loading("Refreshing list...", 1);
      } else {
        throw new Error(response.message || 'Failed to delete gym coupon');
      }
      
    } catch (error) {
      message.error(error.message || "Failed to delete gym coupon");
    } finally {
  ...
  const handleEditCoupon = useCallback((coupon) => {
    navigate(`/vendor/gym/editdiscount/${coupon.id}`, { state: { couponData: coupon } });
  }, [navigate]);
  ...
  const handleStatusToggle = useCallback(async (coupon) => {
    try {
      ...
      if (response.status === 200) {
        message.success(`Gym coupon ${newStatus === 1 ? 'activated' : 'deactivated'} successfully`);
        // Refetch the coupons list to update the UI
        await refetchCoupons();
      } else {
        throw new Error(response.message || 'Failed to update gym coupon status');
      }
      
    } catch (error) {
      message.error(error.message || "Failed to update gym coupon status");
    } finally {
      ...
      render: (_, record) => (
        <div className="gym-cell">
          <div className="gym-name">
            <Typography.Text strong>{record.gym}</Typography.Text>
          </div>
          <div className="gym-id">
            <Typography.Text type="secondary" style={{ fontSize: '12px' }}>
              ID: {record.gymId || 'N/A'}
            </Typography.Text>
          </div>
      ...
      render: (_, record) => {
        ...
        return (
          <Space size="small">
            ...
            <Tooltip title={isDeletingThisCoupon ? "Deleting..." : "Delete Gym Coupon"}>
              <Popconfirm
                title="Are you sure you want to delete this gym coupon?"
    ...
    return (
      <div className="venue-card">
        <div className="page-loading-container">
          <Spin size="large" />
          <div className="page-loading-text">Loading gym coupons...</div>
        </div>
      </div>
    );
  }
        ...
        {couponsFetching && !couponsLoading && (
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              <Spin size="large" />
              <div style={{ marginTop: 8, color: '#666' }}>Refreshing gym coupon list...</div>
            </div>
          </div>
        )}
        {paginatedCoupons?.length > 0 ? (
          ...
          <div className="empty-state-container">
            <div className="empty-state-content">
              ...
              <h3 className="empty-state-title">No Gym Discount Coupons Found</h3>
              <p className="empty-state-description">
                You haven't created any gym discount coupons yet. Click the "ADD GYM DISCOUNT COUPON" button to create your first coupon.
              </p>
              <div className="empty-state-info">
                <div className="info-item">
                  <span className="info-label">Coupon Types:</span>
                  <span className="info-value">Percentage, Fixed Amount</span>
                </div>
                <div className="info-item">
                  <span className="info-label">Features:</span>
                  <span className="info-value">Expiry dates, Gym-specific discounts, Usage limits</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="coupons-footer">
          <div className="pagination-info">
            <Typography.Text type="secondary">
              Showing {startIndex + 1}-{Math.min(startIndex + pageSize, dataSource.length)} of {dataSource.length} gym coupons
            </Typography.Text>
          </div>
        </div>
      )}
...
export default GymDiscountPage;