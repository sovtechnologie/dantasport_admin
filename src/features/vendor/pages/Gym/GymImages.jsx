...
import AddGymImage from "./AddGymImage";
...
const SortableImageCard = ({ 
  ...
  const accessibilityProps = {
    role: 'button',
    tabIndex: 0,
    'aria-label': `Gym Image ${image.display_order || index + 1}, drag to reorder`,
    'aria-describedby': `image-${image.id}-description`,
    'data-testid': `sortable-gym-image-${image.id}`,
  };
    ...
    >
      <Card
        ...
        cover={
          ...
          >
            <img 
              src={image.image_url} 
              alt={`Gym Image ${image.display_order || index + 1}`} 
              className="compact-image"
              loading="lazy"
              onError={(e) => {
                console.warn(`Failed to load gym image ${image.id}:`, e);
            ...
            <div className="compact-overlay">
              <Tooltip title="Click to view full size">
                <Button 
                  ...
                  aria-label="View full size gym image"
                />
              </Tooltip>
            </div>
        ...
        actions={[
          <Tooltip title="Drag to reorder" key="drag">
            <Button 
              ...
              disabled={deletingImageId === image.id || isReordering}
              {...listeners}
              aria-label="Drag to reorder gym image"
            />
          </Tooltip>,
      ...
      >
        ...
        <div id={`image-${image.id}-description`} className="sr-only">
          Gym Image {image.display_order || index + 1} of gym gallery. Use drag handle to reorder.
        </div>
      </Card>
    </div>
  );
...
const GymImagesPage = () => {
  ...
  const { data: galleryList, loading: galleryLoading, error: galleryError } = useFetchGalleryImage({ venueId: selectedGymId || 1, type: 3 }); // type: 3 for gym
  ...
  useEffect(() => {
    ...
    if (galleryError) {
      message.error("Failed to load gym images");
    }
  }, [gymError, galleryError]);
    ...
    try {
      ...
      const updatePromises = imagesToUpdate.map(async (imageData, retryCount = 0) => {
        try {
          ...
          formData.append("venueId", String(selectedGymId));
  ...
  const handleAddImage = useCallback(() => {
    if (sortedImages?.length >= 6) {
      message.warning("Maximum of 6 images allowed per gym. Please delete an existing image to add a new one.");
      return;
    }
  ...
  const handleEditImage = useCallback((image) => {
    console.log("✏️ Editing gym image:", image);
    setSelectedImage(image);
    setShowEditModal(true);
  }, []);

  const handleViewImage = useCallback((image) => {
    console.log("👁️ Viewing gym image:", image);
  ...
  const handleDeleteImage = async (image) => {
    try {
      ...
      const payload = { imageId: image.id, venueId: selectedGymId };
  ...
  // Show loading state when no gym is selected, when gym is changing, or when images are loading
  if (!selectedGymId || isGymChanging || (selectedGymId && galleryLoading) || (selectedGymId && !galleryList)) {
    return (
      <div className="venue-card">
        ...
        <h3 className="venue-title">
          {selectedGym?.gym_name || "Select a gym to view images"}
        </h3>
        ...
        <div className="page-loading-container">
          ...
          <div className="page-loading-text">
            {!selectedGymId 
              ? "Please select a gym to view images" 
              : isGymChanging 
                ? "Switching gym..." 
                : selectedGymId && !galleryList
                  ? "Fetching gym images..."
                  : "Loading images..."
            }
          </div>
    ...
    <div className="venue-card">
      <div className="venue-toolbar">
        ...
        <Button 
          type="primary" 
          className="add-btn"
          disabled={!selectedGymId || (sortedImages?.length >= 6) || galleryLoading || isReordering}
            ...
            <div className="empty-state-container">
              <div className="empty-state-content">
                ...
                <p className="empty-state-description">
                  This gym doesn't have any images yet. Click the "Add Gym Image" button to upload your first image.
                </p>
                <div className="empty-state-info">
                  <div className="info-item">
                    <span className="info-label">Maximum Images:</span>
                    <span className="info-value">6 images per gym</span>
                  </div>
                  <div className="info-item">
        ...
        {viewingImage && (
          <div style={{ textAlign: 'center' }}>
            <Image
              ...
              alt={`Gym Image ${viewingImage.display_order}`}
              style={{ 
                maxWidth: '100%', 
                maxHeight: '70vh',
                objectFit: 'contain'
            ...
            <div style={{ 
              ...
              <div style={{ marginBottom: '8px' }}>
                <strong>Gym:</strong> {selectedGym?.gym_name}
              </div>
              <div>
