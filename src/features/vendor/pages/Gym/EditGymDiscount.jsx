...
export default function EditGymDiscount() {
    ...
    // Fetch gym list
    const { data: gymList, loading: gymLoading, error: gymError } = useFetchGymList(vendorId);
    ...
    useEffect(() => {
        console.log('📋 EditGymDiscount - Location state:', location.state);
        console.log('📋 EditGymDiscount - Coupon ID from params:', couponId);
        
        if (location.state?.couponData) {
            const data = location.state.couponData;
            console.log('📋 EditGymDiscount - Received coupon data:', data);
            setCouponData(data);
            setSelectedGymId(data.gymId);
            
            // Set form values
            form.setFieldsValue({
                selectGym: data.gymId,
    ...
    useEffect(() => {
        if (gymList?.result?.length && !selectedGymId && !couponData) {
            setSelectedGymId(gymList.result[0].Id);
            form.setFieldsValue({ selectGym: gymList.result[0].Id });
        }
    }, [gymList, selectedGymId, form, couponData]);
    ...
    const onFinish = async (values) => {
        try {
            ...
            console.log('📤 EditGymDiscount - API Payload:', payload);
            console.log('📤 EditGymDiscount - Selected Gym ID:', selectedGymId);
            console.log('📤 EditGymDiscount - Coupon Data:', couponData);

            // Call API
            const response = await updateCouponMutation.mutateAsync(payload);
            
            if (response.status === 200) {
                message.success("Gym discount coupon updated successfully!");
                ...
                throw new Error(response.message || 'Failed to update gym coupon');
            }
        } catch (error) {
            console.error('Error updating gym coupon:', error);
            message.error(error.message || "Failed to update gym discount coupon. Please try again.");
        } finally {
            ...
            <div className="coupon-header-compact">
                <div className="header-left">
                    ...
                    <div className="header-title">
                        ...
                        <Title level={3} className="coupon-title-compact">Edit Gym Discount Coupon</Title>
                    </div>
                </div>
                ...
                >
                    ...
                    <div className="form-actions-compact">
                        <Button 
                            ...
                            block
                        >
                            {isSubmitting || updateCouponMutation.isPending ? 'Updating Gym Coupon...' : 'Update Gym Discount Coupon'}
                        </Button>
                    </div>
                </Form>
            </Card>
