...
const PermissionTreeView = ({ 
  ...
  const hasPermissions = initialPermissions.permissions && initialPermissions.permissions.length > 0;
  ...
  useEffect(() => {
    if (hasPermissions) {
      ...
      permissionCategories.forEach(category => {
        ...
        const apiPermission = initialPermissions.permissions.find(p => 
          p.name.toLowerCase() === category.label.toLowerCase()
        );
        
        if (apiPermission) {
          // Map API status to permission type
          const statusMap = { 0: 'hide', 1: 'read', 2: 'write' };
          apiPermissions[category.key] = statusMap[apiPermission.status] || 'hide';
        } else {
  ...
  useEffect(() => {
    const hasChanges = Object.keys(permissions).some(key => 
      permissions[key] !== (initialPermissions[key] || 'read')
    );
  ...
  const handleUpdateRole = async () => {
    try {
      ...
      const apiPermissions = permissionCategories.map(category => {
        ...
        const statusValue = permissionTypes.find(p => p.key === permissionType)?.value || 0;
  ...
  const handleReset = () => {
    const defaultPermissions = {};
    permissionCategories.forEach(category => {
      defaultPermissions[category.key] = initialPermissions[category.key] || 'read';
    });
    setPermissions(defaultPermissions);
    setHasChanges(false);
  };
    ...
    return (
      <div className="permission-treeview-loading">
        ...
        <div className="loading-text">
          {hasPermissions ? 'Refreshing permissions...' : 'Loading permissions...'}
        </div>
      </div>
    );
  }
    ...
    <div className="permission-treeview">
      <div className="permission-header">
        <h4 className="permission-title">
          {hasPermissions ? `Manage Permissions for ${memberName}` : `Add Role for ${memberName}`}
        </h4>
      ...
      {!hasPermissions && (
        <div className="no-permissions-message">
          <p>No permissions have been assigned to this member yet. Set the permissions below and click "ADD ROLE" to assign them.</p>
        </div>
      )}
              ...
              {permissionTypes.map(permission => (
                <div 
                  ...
                  className={`permission-option ${
                    permissions[category.key] === permission.key ? 'selected' : ''
                  }`}
                >
                  <Radio
                    checked={permissions[category.key] === permission.key}
                    onChange={() => handlePermissionChange(category.key, permission.key)}
                    disabled={saving}
                  />
